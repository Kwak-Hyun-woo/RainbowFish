...
class MeshContainer(object):
    ...
    def _load_from_file_glb(self, file_path):
        import trimesh
        try:
            mesh = trimesh.load(file_path, file_type='glb', process=False)
            ...
            print(f"[ERROR] Failed to load .glb file: {file_path}")
            print(f"Reason: {e}")
            raise e
    ...
    def load_from_file(self, file_path, dataset=''):
        ...
        if filename.endswith('mat'):
            ...
            self._load_from_file_glb(file_path)
        return self
